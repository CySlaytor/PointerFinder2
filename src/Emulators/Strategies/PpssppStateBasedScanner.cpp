#include "PpssppStateBasedScanner.h"
...
namespace PointerFinder2::Emulators::StateBased {
    ...
    // Reads the captured PSP memory snapshot and builds an index map of all active pointers.
    void PpssppStateBasedScanner::buildPointerMap(const std::vector<uint8_t>& memory, std::atomic<bool>& token) {
        m_pointerMap.clear();
    ...
    SourceResults PpssppStateBasedScanner::findSourcesForValue(uint32_t value) const {
        SourceResults res;
